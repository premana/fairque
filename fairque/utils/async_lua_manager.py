"""Async Lua script management utilities for FairQueue."""
...
class AsyncLuaScriptManager:
    """Manages Lua script loading, caching, and execution for async FairQueue operations."""
    ...
    def __init__(self, redis_client: redis.Redis, script_dir: Optional[Path] = None) -> None:
        ...
        self.loaded_scripts: Dict[str, AsyncScript] = {}
        self.script_hashes: Dict[str, str] = {}

    async def load_script(self, script_name: str) -> AsyncScript:
        ...
        try:
            ...
            logger.debug(f"Loaded async Lua script '{script_name}' with hash {script_hash}")
            return script
    ...
    async def execute_script(
        ...
        """Execute a loaded Lua script asynchronously.
        ...
        try:
            script = await self.load_script(script_name)
            result = await script(keys=keys, args=args)

            logger.debug(
                f"Executed async script '{script_name}' with {len(keys)} keys and {len(args)} args"
            )
            return result
    ...
    async def reload_script(self, script_name: str) -> AsyncScript:
        ...
        logger.info(f"Reloading async Lua script '{script_name}'")
        return await self.load_script(script_name)

    async def load_all_scripts(self) -> None:
        ...
        logger.info(f"Loading {len(script_files)} async Lua scripts from {self.script_dir}")

        for script_path in script_files:
            script_name = script_path.stem
            try:
                await self.load_script(script_name)
            except LuaScriptError as e:
                logger.error(f"Failed to load async script '{script_name}': {e}")
                raise
    ...
    def clear_cache(self) -> None:
        ...
        logger.info("Clearing async Lua script cache")
